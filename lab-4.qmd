...
```{r areaaaaaaaaaaaaaaaaaaaaaallllllllllllllllllyyyyylooooooooooooooooooonglabel}
1+1
```
...
I assume chunk label 3 was not supposed to render since there is spaces in the label but the assignment format has errors preventing me from reaching that conclusion.
Chunk label 1 and 4 are best because they are simple and easy to understand. Chunk 2 lacks brevity.
...
-   `echo`: displays or hides code in rendered document
-   `warning`: displays or hides any warning produced by the code in the rendered document
-   `eval`: runs code so plot is displayed in rendered document
...
fig-width adjusts the width of the figure and fig-asp controls the height to width ratio of the figure.
...
Error allows for control over whether a chunk of code with an arror will stop the document from rendering or not.
